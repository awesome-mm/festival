...
<%	 
  	 	...
  	 	sql_m = "select c_no from tbl_puzzle where c_year = 2021 and c_use = 0 and c_member_no = "& session("session_no")
		...
		If rs_m.EOF Then 
			...
			sql_p = sql_p & " ,(select max(c_order) as my_no from tbl_puzzle where c_year = 2021 and c_use = 0 and c_puzzle_no = a.c_puzzle_no ) as my_no "
			sql_p = sql_p & " from tbl_puzzle a where a.c_year = 2021 and a.c_use = 0 ) t where t.my_no < 2500 order by t.c_puzzle_no"
			...
			in_sql = in_sql & "2021" &","& now_p &","& my_order &","& my_garo &","& my_sero  &","& session("session_no") &",'"& session("session_id")  &"','"& c_content &"',"
