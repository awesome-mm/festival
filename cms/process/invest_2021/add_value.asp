	...
	function go_next(){
		if(now_value + 1 > <%=ai%>){
			...
			for (var i = now_value + 1; i < now_value + 9 ; i++) {
					...
					t1 = t1 + 1
					let_go = let_go +'<a href="/cms/process/invest_2021/view.asp?c_show_no=71&c_check_no=64&c_relation=809&c_relation2=903&c_team_no='+ c_no[i] + '&c_festival_type=<%=c_festival_type%>&s_text=<%=s_text%>">	'
